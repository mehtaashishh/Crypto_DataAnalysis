...
def get_ethereum_data(start_date, end_date):
    """Fetches Ethereum price data from the CryptoCompare API."""
    ...
    while True:
        url = f"https://min-api.cryptocompare.com/data/v2/histoday?fsym=ETH&tsym=USD&limit=2000&toTs={current_timestamp}"
...
def create_interactive_ethereum_chart(data, start_date):
    """Creates an interactive Ethereum price chart with Plotly."""
    if not data:
    ...
    merge_day = (merge_date - start_datetime).days + 1
    fig.add_vline(x=merge_day, line_width=1, line_dash="dash", line_color="grey", annotation_text=f"The Merge {merge_date.year}", annotation_position="top left")
    ...
    fig.update_layout(
        title='Interactive Ethereum Price History - Log-Log Scale with Quantile Regression',
        xaxis_title='Date',
    ...
    fig.write_html("interactive_ethereum_chart.html")
    print("Interactive chart saved to interactive_ethereum_chart.html")
    ...
    df_table.to_csv('ethereum_price_predictions.csv', index=False)
    # --- End Create CSV ---
...
if __name__ == "__main__":
    ...
    price_data = get_ethereum_data(start_date, end_date)
    if price_data:
        create_interactive_ethereum_chart(price_data, start_date)
